...
export default function App() {
  ...
  const handleGrade = (quality) => {
    ...
    supabase.from('words').update({ interval: reviewData.interval, repetitions: reviewData.repetitions, next_review: reviewData.next_review }).eq('id', currentCard.id).catch(() => triggerFeedback('⚠️ 网络断开，离线修改中'));
  ...
  const renderHeader = (activeTab) => (
    <header className="w-full max-w-4xl flex justify-between items-center mb-6 sm:mb-8 shrink-0 px-2 sm:px-0">
      ...
      <div className="flex bg-white rounded-full p-1 shadow-sm border border-slate-200/80 overflow-x-auto">
        <button onClick={() => { setCurrentView('flashcard'); setIsFlipped(false); const shuffled = shuffleArray(filteredCards); setFilteredCards(shuffled); setCurrentIndex(0); }} className={`px-3 sm:px-4 py-1.5 rounded-full text-[10px] sm:text-xs font-bold flex items-center gap-1 shrink-0 transition-colors ${activeTab === 'flashcard' ? 'bg-[#EBF5F0] text-[#0D9488]' : 'text-slate-500 hover:bg-slate-50'}`}>
          <span>🎴</span> 传统背卡
        </button>
        <button onClick={() => { setCurrentView('dictation'); setQuizStatus('waiting'); setQuizInput(''); const shuffled = shuffleArray(quizPool); setQuizPool(shuffled); setCurrentIndex(0); }} className={`px-3 sm:px-4 py-1.5 rounded-full text-[10px] sm:text-xs font-bold flex items-center gap-1 shrink-0 transition-colors ${activeTab === 'dictation' ? 'bg-[#EBF5F0] text-[#0D9488]' : 'text-slate-500 hover:bg-slate-50'}`}>
          <span>🎯</span> 听音拼写
        </button>
        <button onClick={() => { setCurrentView('hall'); setSelectedLibPack(null); }} className={`px-3 sm:px-4 py-1.5 rounded-full text-[10px] sm:text-xs font-bold flex items-center gap-1 ml-1 shrink-0 transition-colors ${activeTab === 'hall' || activeTab === 'list' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}>
                ...
                <div className="w-full bg-[#F0FDF4] border border-[#DCFCE7] rounded-2xl p-3 sm:p-4 mb-4 flex items-center justify-between shadow-sm shrink-0">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] sm:text-[10px] text-slate-500 font-medium">下次复习: <strong className="text-[#166534]">{filteredCards[currentIndex]?.interval || 1}天后</strong></p>
                    </div>
                  </div>
                ...
                >
                  <div className={`relative w-full h-full text-center transition-transform duration-500 [transform-style:preserve-3d] ${isFlipped ? '[transform:rotateY(180deg)]' : ''}`}>
                    <div className="absolute inset-0 w-full h-full bg-white rounded-[32px] shadow-[0_12px_32px_rgba(15,23,42,0.06)] border border-slate-100 p-8 sm:p-12 flex flex-col items-center justify-center relative overflow-hidden" style={{ backfaceVisibility: 'hidden' }}>
                      <button onClick={(e) => handleArchiveCard(filteredCards[currentIndex].id, e)} className="absolute top-5 right-5 text-[10px] sm:text-xs bg-slate-50 text-slate-400 hover:text-rose-500 hover:bg-rose-50 px-3 py-1 rounded-full border border-slate-200/60 font-bold transition-colors z-10 shadow-sm">封印 🐾</button>
                      <div className="flex flex-col items-center justify-center flex-1 my-auto w-full">
                        <div className="flex items-center justify-center gap-3 mb-2 flex-wrap">
                          <h2 className="text-5xl sm:text-7xl font-black text-[#0F172A] tracking-tight">{filteredCards[currentIndex]?.word}</h2>
                          <SoundWaveButton onClick={(e) => playSpeech(filteredCards[currentIndex]?.word, e)} size="medium" isSpeaking={speakingText === filteredCards[currentIndex]?.word} />
                        </div>
                        <p className="text-xl sm:text-2xl text-slate-400 font-light mt-1">{filteredCards[currentIndex]?.phonetic}</p>
                      </div>
                    ...
                    <div className="absolute inset-0 w-full h-full bg-slate-50/90 rounded-[32px] shadow-[0_12px_32px_rgba(15,23,42,0.06)] border border-slate-200/80 p-8 sm:p-12 flex flex-col items-center justify-center [backface-visibility:hidden] [transform:rotateY(180deg)]">
                      <div className="flex flex-col items-center justify-center flex-1 my-auto w-full">
                        <h2 className="text-4xl sm:text-5xl font-black text-[#0F172A] mb-6">{filteredCards[currentIndex]?.translation}</h2>
                        <div className="flex items-center justify-center gap-3 mb-2 max-w-full px-2">
                          <p className="text-sm sm:text-lg text-slate-700 font-medium break-words leading-relaxed text-center flex-1">"{filteredCards[currentIndex]?.sentence}"</p>
                          <SoundWaveButton onClick={(e) => playSpeech(filteredCards[currentIndex]?.sentence, e)} size="small" isSpeaking={speakingText === filteredCards[currentIndex]?.sentence} />
                        </div>
                        <p className="text-xs sm:text-sm text-slate-400 mt-2">({filteredCards[currentIndex]?.translation_cn})</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {!quizPool[currentIndex] ? (
              ...
              <div className="w-full bg-white rounded-[32px] shadow-[0_12px_32px_rgba(15,23,42,0.06)] border border-slate-100 p-6 sm:p-8 flex flex-col items-center relative shrink-0 min-h-[380px] overflow-visible">
                ...
                <div className="w-full flex justify-end items-center mb-4 shrink-0">
                  <div className="flex gap-2">
                    <span className="bg-slate-50 text-slate-600 border border-slate-200/80 text-[10px] sm:text-xs px-3 py-1.5 rounded-full shadow-sm font-bold">
                      🎯 连对: <strong className="text-[#D97706] ml-1">{quizPool[currentIndex].streak_correct || 0}</strong>
                    </span>
                ...
                <div className="flex flex-col items-center justify-center flex-1 w-full my-auto py-2">
                  ...
                  <h2 className="text-3xl sm:text-4xl font-black text-[#0F172A] mb-3 tracking-wide text-center">{quizPool[currentIndex].translation}</h2>
                  <SoundWaveButton onClick={() => playSpeech(quizPool[currentIndex].word)} size="medium" className="my-1" isSpeaking={speakingText === quizPool[currentIndex].word} />
                  <p className="text-xs text-slate-400 font-mono mt-3">级别: {quizPool[currentIndex].level}  |  场景: {quizPool[currentIndex].category}</p>
                </div>
                    ...
                    {quizStatus === 'correct' && (
                      <div className="w-full flex flex-col items-center max-w-xl mx-auto animate-pulse">
                        <div className="bg-[#F0FDF4] w-full rounded-2xl p-4 sm:p-5 text-center border border-[#DCFCE7] shadow-sm flex flex-col items-center justify-center">
                          ...
                          <div className="text-2xl sm:text-3xl text-[#166534] font-black tracking-widest uppercase">{quizPool[currentIndex]?.word}</div>
                        </div>
                      </div>
                    )}
                          ...
                          <div className="bg-white rounded-xl p-3 sm:p-4 text-xs font-mono shadow-sm">
                            <div className="flex items-center gap-2 py-1.5 border-b border-dashed border-slate-100">
                              ...
                              <div className="flex flex-wrap text-sm sm:text-base tracking-widest">
                                {getDiff(quizInput, quizPool[currentIndex].word).map((d, idx) => {
                            ...
                            <div className="flex items-center gap-2 py-1.5 mt-1">
                              ...
                              <div className="flex flex-wrap text-sm sm:text-base tracking-widest items-center gap-1.5">
                                ...
                                <SoundWaveButton onClick={(e) => playSpeech(quizPool[currentIndex].word, e, true)} size="small" isSpeaking={speakingText === quizPool[currentIndex].word} />
                              </div>
                            </div>
                          </div>
              ...
              {globalSearchQuery ? (
                <div className="w-full space-y-4 px-2">
                  <div className="text-xs text-slate-500 font-bold px-2 flex justify-between items-center">
                    <span>搜到 {filterListCards(rawCards, globalSearchQuery).length} 个相关单词：</span><span className="text-slate-400">展示级别与全部属性</span>
                  </div>
                  {filterListCards(rawCards, globalSearchQuery).slice(0, globalVisibleCount).map((card) => (
                  ...
                  {filterListCards(rawCards, globalSearchQuery).length > globalVisibleCount && (
                    <div className="text-center py-4"><button onClick={() => setGlobalVisibleCount(prev => prev + 20)} className="bg-[#F0FDF4] text-[#166534] border border-[#DCFCE7] px-6 py-2 rounded-full text-sm font-bold hover:bg-[#DCFCE7]">查看更多结果 👇</button></div>
                  )}
                </div>
              ) : (
                <>
            ...
            <div className="w-full max-w-5xl bg-white border border-slate-200/80 rounded-[32px] shadow-[0_12px_32px_rgba(15,23,42,0.05)] overflow-hidden flex flex-col min-h-[500px] mb-8">
              ...
              <div className="flex-1 overflow-x-auto p-2 sm:p-6">
                <div className="min-w-[500px]">
                  ...
                  {filterListCards((rawCards || []).filter(c => c && c.level === selectedLibPack.level && c.category === selectedLibPack.category), listSearchQuery).slice(0, listVisibleCount).map((card) => (
                  ...
                  {filterListCards((rawCards || []).filter(c => c && c.level === selectedLibPack.level && c.category === selectedLibPack.category), listSearchQuery).length > listVisibleCount && (
                    <div className="text-center py-6 border-t border-slate-100"><button onClick={() => setListVisibleCount(prev => prev + 20)} className="bg-[#F0FDF4] text-[#166534] border border-[#DCFCE7] px-6 py-2 rounded-full text-sm font-bold hover:bg-[#DCFCE7]">加载更多 👇</button></div>
                  )}
                </div>
              </div>
            </div>
...
function SoundWaveButton({ onClick, className = '', size = 'medium', isSpeaking = false }) {
  const [waveState, setWaveState] = useState('idle');
  useEffect(() => {
    if (isSpeaking) setWaveState('active');
    else if (waveState === 'active') {
      setWaveState('fading');
      const timer = setTimeout(() => setWaveState('idle'), 800);
      return () => clearTimeout(timer);
  ...
  const sizeClasses = { small: 'w-9 h-7 sm:w-11 sm:h-8 px-1', medium: 'w-14 h-9 sm:w-18 sm:h-10 px-2', large: 'w-48 sm:w-60 h-14 sm:h-18 px-3' }[size] || 'w-14 h-9 px-2';
  const strokeClass = { active: size === 'large' ? 'stroke-[3.5px]' : 'stroke-[2.5px]', fading: size === 'large' ? 'stroke-[2.2px]' : 'stroke-[1.8px]', idle: 'stroke-[2px]' }[waveState];
  const opacityClass = { active: 'opacity-100 transition-opacity duration-300 ease-in', fading: 'opacity-70 transition-all duration-800 ease-out', idle: 'opacity-85 transition-all duration-800 ease-out' }[waveState];
  return (
    <button type="button" onClick={(e) => { if (e && e.stopPropagation) e.stopPropagation(); if (onClick) onClick(e); }} className={`relative rounded-full bg-[#EFECE4] border border-[#DCD6CA] shadow-sm hover:shadow-md transition-all active:scale-95 flex items-center justify-center overflow-hidden shrink-0 select-none group ${sizeClasses} ${className}`}>
      <style>{`@keyframes pitchWave1 { 0% { transform: translateX(0px) scaleY(0.5); } 25% { transform: translateX(-25px) scaleY(1.6); } 50% { transform: translateX(-50px) scaleY(0.7); } 75% { transform: translateX(-75px) scaleY(1.4); } 100% { transform: translateX(-100px) scaleY(0.5); } } @keyframes pitchWave2 { 0% { transform: translateX(-100px) scaleY(0.7); } 33% { transform: translateX(-66px) scaleY(1.3); } 66% { transform: translateX(-33px) scaleY(0.6); } 100% { transform: translateX(0px) scaleY(0.7); } } @keyframes pitchWave3 { 0% { transform: translateX(0px) scaleY(0.4); } 50% { transform: translateX(-40px) scaleY(1.7); } 100% { transform: translateX(-80px) scaleY(0.4); } } .wave-pitch-1 { animation: pitchWave1 0.75s infinite ease-in-out; } .wave-pitch-2 { animation: pitchWave2 0.95s infinite ease-in-out; } .wave-pitch-3 { animation: pitchWave3 0.65s infinite ease-in-out; } .wave-idle-1 { animation: pitchWave1 4.0s infinite linear; } .wave-idle-2 { animation: pitchWave2 4.5s infinite linear; } .wave-idle-3 { animation: pitchWave3 3.5s infinite linear; }`}</style>
      <div className="w-full h-full flex items-center justify-center relative overflow-hidden">
        <svg viewBox="0 0 300 60" className="w-[150%] h-full shrink-0 overflow-visible">
          <path d="M 0 30 Q 25 10, 50 30 T 100 30 T 150 30 T 200 30 T 250 30 T 300 30 T 350 30 T 400 30" fill="none" stroke="#4A9A74" strokeLinecap="round" className={`origin-center ${waveState === 'active' ? 'wave-pitch-1' : 'wave-idle-1'} ${strokeClass} ${opacityClass}`} />
          <path d="M 0 30 Q 25 50, 50 30 T 100 30 T 150 30 T 200 30 T 250 30 T 300 30 T 350 30 T 400 30" fill="none" stroke="#D4A017" strokeLinecap="round" className={`origin-center ${waveState === 'active' ? 'wave-pitch-2' : 'wave-idle-2'} ${strokeClass} ${opacityClass}`} />
          <path d="M 0 30 Q 25 20, 50 40 T 100 30 T 150 30 T 200 30 T 250 30 T 300 30 T 350 30 T 400 30" fill="none" stroke="#38BDF8" strokeLinecap="round" className={`origin-center ${waveState === 'active' ? 'wave-pitch-3' : 'wave-idle-3'} ${strokeClass} ${opacityClass}`} />
        </svg>
      </div>
    </button>

...
export default function LibraryView({
  ...
  // 🌟 核心智能搜索匹配引擎（根据输入语言自动隔离，彻底杜绝 n. v. 词性干扰）
  const filterCardsByQuery = (cards, searchQuery) => {
    ...
    return cards.filter((card) => {
      if (isCnQuery) {
        ...
        return cnMatch || cnSentenceMatch;
      } else {
        // 2. 输入为纯英文：只搜英文单词首字母(前缀)与英文例句，绝不碰撞中文翻译，杜绝词性标记干扰！
        const enWordMatch = card.word && card.word.toLowerCase().startsWith(query);
        const enSentenceMatch = card.sentence && card.sentence.toLowerCase().includes(query);
        return enWordMatch || enSentenceMatch;
      }
    });
  };

    ...
    <div className="w-full max-w-3xl flex-1 flex flex-col justify-center pb-8 sm:pb-12">
      {!currentQuizCard ? (
        <div className="w-full bg-[#EFECE4] border border-[#DCD6CA] rounded-[32px] shadow-sm p-12 text-center">
          ...
          <p className="text-gray-600 font-bold mb-4">考核队列空空如也，太厉害了！</p>
          <button onClick={onChangePack} className="bg-[#A3C9B8] text-[#2D4A3E] px-6 py-2 rounded-xl font-bold shadow-sm">换个主题继续</button>
        </div>
      ) : (
        /* 🌟 核心：换为护眼暖燕麦色背景 #EFECE4 */
        <div className="w-full bg-[#EFECE4] border border-[#DCD6CA] rounded-[32px] shadow-md p-6 sm:p-8 flex flex-col items-center relative shrink-0 min-h-[380px] overflow-visible">
          <button onClick={onChangePack} className="absolute top-6 left-6 text-gray-500 text-xs sm:text-sm flex items-center gap-1 hover:text-gray-700 transition-colors bg-white px-3 py-1.5 rounded-full shadow-sm z-10">
            🔙 换包
          </button>
          
          <div className="w-full flex justify-end items-center mb-4 shrink-0">
            <div className="flex gap-2">
              <span className="bg-white text-gray-600 border border-gray-200 text-[10px] sm:text-xs px-3 py-1.5 rounded-full shadow-sm">
          ...
          <div className="flex flex-col items-center justify-center flex-1 w-full my-auto py-2">
            <p className="text-[11px] sm:text-xs text-[#D4A017] font-bold mb-3 tracking-wider bg-[#FFF8E1] border border-[#FFE8B3] px-4 py-1.5 rounded-full">👇 请听音并拼写</p>
            <h2 className="text-3xl sm:text-4xl font-black text-slate-800 mb-3 tracking-wide text-center">{currentQuizCard.translation}</h2>
          ...
          <div className="w-full mt-auto pt-4 border-t border-[#DCD6CA] shrink-0">
              ...
              {quizStatus === 'waiting' ? (
                <div className="w-full flex gap-2 sm:gap-3 max-w-xl mx-auto">
                  <input 
                    ...
                    className="flex-1 bg-white border-2 border-[#DCD6CA] rounded-xl px-4 py-3 sm:py-4 text-lg sm:text-xl font-bold text-center tracking-widest text-slate-800 focus:outline-none focus:border-[#A3C9B8] shadow-inner placeholder:text-gray-300"

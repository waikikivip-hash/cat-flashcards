...
export default function LibraryView({
        ...
        <div className="text-center mb-6 mt-2 pt-10 sm:pt-0">
          ...
          <div className="w-full max-w-xl mx-auto mb-6 px-2">
            <div className="relative">
              <input 
                ...
                className="w-full px-5 py-3 bg-[#EFECE4] border-2 border-[#A3C9B8] rounded-2xl text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-[#A3C9B8]/50 shadow-sm font-medium text-slate-800 placeholder:text-gray-400"
              />
              {globalSearchQuery && (
                <button onClick={() => setGlobalSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 font-bold text-sm bg-white rounded-full w-6 h-6 flex items-center justify-center">✕</button>
              )}
            </div>
          </div>
          ...
          {!globalSearchQuery && (
            <div className="flex flex-col items-center gap-3">
              ...
              <div className="bg-[#EFECE4] rounded-full p-1 shadow-sm flex flex-wrap justify-center border border-[#DCD6CA]">
                {getAvailableLevels().map((lvl) => (
                  <button key={lvl} onClick={() => setHallLevel(lvl)} className={`px-6 py-2 text-sm rounded-full font-bold transition-colors ${hallLevel === lvl ? 'bg-[#A3C9B8] text-[#2D4A3E] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>
                    {lvl}
                  </button>
                ))}
              </div>
        ...
        {globalSearchQuery ? (
          <div className="w-full space-y-4 px-2">
            ...
            {displayResults.map((card) => (
              <div key={card.id} className="bg-[#EFECE4] rounded-2xl p-4 sm:p-5 border border-[#DCD6CA] shadow-sm flex flex-col gap-3 hover:shadow-md">
                <div className="flex justify-between items-center text-xs">
                  <div className="flex gap-2 items-center">
                    ...
                    {card.is_archived && <span className="bg-white/80 text-gray-400 font-bold px-2 py-0.5 rounded-md text-[10px]">已封印 🐾</span>}
                  </div>
                  <div className="text-gray-500 text-[11px]">连对: <strong className="text-[#D4A017]">{card.streak_correct || 0}</strong>次 | 间隔: <strong className="text-[#4A9A74]">{card.interval || 1}</strong>天</div>
                </div>
                <div className="flex justify-between items-start">
                  ...
                  <button onClick={(e) => handleArchiveCard(card.id, e)} className={`text-xs px-3 py-1.5 rounded-full font-bold ${card.is_archived ? 'bg-white/60 text-gray-400' : 'text-[#D84C4C] bg-[#FFEBEB]'}`}>
                    {card.is_archived ? '已归档' : '封印 🐾'}
                  </button>
                </div>
                {card.sentence && (
                  <div className="bg-white/80 rounded-xl p-3 text-xs border border-gray-100/80">
            ...
            {globalSearchResults.length === 0 && <div className="text-center py-12 text-gray-400 font-bold text-sm bg-[#EFECE4] rounded-2xl border border-[#DCD6CA]">没有找到符合 “{globalSearchQuery}” 的单词 😿</div>}
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6 pb-10">
              {getLibraryPacks().filter(p => p.level === hallLevel).map((pack) => (
                <div key={`${pack.level}-${pack.category}`} onClick={() => { setSelectedLibPack(pack); setListSearchQuery(''); setListVisibleCount(20); setCurrentView('list'); }} className="bg-[#EFECE4] border border-[#DCD6CA] rounded-3xl shadow-sm py-8 flex flex-col items-center cursor-pointer hover:-translate-y-1 transition-transform">
                  <div className="text-4xl mb-4">📦</div>
                  <h3 className="text-lg font-bold text-slate-800 mb-2">{pack.category}</h3>
                  <span className="text-xs text-[#4A9A74] font-bold">共 {pack.count} 词</span>
                </div>
              ))}
  ...
  if (currentView === 'list' && selectedLibPack) {
    ...
    return (
      <div className="w-full max-w-5xl bg-[#EFECE4] border border-[#DCD6CA] rounded-[32px] shadow-sm overflow-hidden flex flex-col min-h-[500px] mb-8">
        <div className="flex flex-col sm:flex-row justify-between items-center p-4 border-b border-[#DCD6CA] shrink-0 gap-3">
          <div className="flex w-full sm:w-auto justify-between items-center gap-2">
            <button onClick={() => setCurrentView('hall')} className="text-gray-600 text-sm flex items-center gap-1 border border-gray-300 px-4 py-1.5 rounded-full shrink-0 bg-white hover:bg-gray-50 transition-colors shadow-sm">🔙 返回大厅</button>
          ...
          <input 
            ...
            className="w-full sm:w-56 px-4 py-2 bg-white border border-[#DCD6CA] rounded-full text-sm focus:outline-none focus:border-[#A3C9B8] text-slate-800"
          />
        </div>
        
        <div className="flex-1 overflow-x-auto p-2 sm:p-6">
          <div className="min-w-[500px]">
            <div className="grid grid-cols-5 text-center text-sm font-bold text-gray-500 mb-4 pb-2 border-b border-[#DCD6CA]">
            ...
            {displayList.map((card) => (
              <div key={card.id} className="grid grid-cols-5 text-center items-center py-3 border-b border-[#DCD6CA]/50 hover:bg-white/50 transition-colors">
                  ...
                  <button 
                    ...
                    className={`text-xs px-3 py-1.5 rounded-full font-bold transition-colors ${
                      card.is_archived 
                        ...
                        : 'text-gray-500 bg-white hover:text-rose-500 hover:bg-rose-50 border border-gray-200'
                    }`}
                  >
                    {card.is_archived ? '已归档' : '封印 🐾'}
                  </button>
            ...
            {searchedList.length > listVisibleCount && (
              <div className="text-center py-6 border-t border-[#DCD6CA]">
